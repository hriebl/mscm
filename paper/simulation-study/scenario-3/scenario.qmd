---
title: "Simulation Study, Scenario 3"
author: "Hannes Riebl"
format: gfm
params:
  nsites: 40
...
df <- readRDS(here("paper", "simulation-study", "scenario-3", "rtg-2300-col-df.rds"))

gb <- liesel(
  response = y,
  predictors = list(
    eta = predictor(
      formula = ~ -1 + apa_spruce + apa_douglas + s(x, y, k = 40, bs = "gp", m = c(-3, 0.008047973)),
...
pdt_vars["eta_np0_tau2"].value = 1.0
...
occ.simulate_model(sim_model, seed=r.seeds[0], skip=["eta_np0_tau2_log_prob"])
...
builder.add_kernel(lsl.tau2_gibbs_kernel(mcmc_model.groups()["eta_np0"]))
